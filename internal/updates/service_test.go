...
func TestEmbeddedManifestAndVersionComparison(t *testing.T) {
	...
	if status.CurrentVersion != "1.3.0" || status.UpdateAvailable || len(status.Changelog) == 0 {
...
func TestGitHubReleaseAPIAndInstallerSelection(t *testing.T) {
	...
	if apiURL != "https://api.github.com/repos/brunooboy/bruno-browser/releases/tags/v1.3.0" {
	...
	asset, err := selectInstallerAsset([]githubReleaseAsset{
		{Name: "Bruno-Browser-Setup-amd64.exe", BrowserDownloadURL: "https://github.com/brunooboy/bruno-browser/releases/download/v1.3.0/amd64.exe", Digest: digest, Size: 20, State: "uploaded"},
		{Name: "Bruno.Browser.Setup.exe", BrowserDownloadURL: "https://github.com/brunooboy/bruno-browser/releases/download/v1.3.0/Bruno.Browser.Setup.exe", Digest: digest, Size: 30, State: "uploaded"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestVerifiedDownloadChecksSizeAndSHA256(t *testing.T) {
	...
	result, err := verifiedDownload(path, "1.3.0", Asset{
		Name: filepath.Base(path), Size: int64(len(payload)), SHA256: hex.EncodeToString(digest[:]),
	})
	if err != nil {
		t.Fatal(err)
	}
	if !result.Ready || result.Version != "1.3.0" || result.Bytes != int64(len(payload)) {
		t.Fatalf("unexpected verified download: %#v", result)
	}
	if _, err := verifiedDownload(path, "1.3.0", Asset{
